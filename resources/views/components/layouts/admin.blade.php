    ...
    <div class="min-h-screen flex bg-white">
        ...
        <aside :class="sidebarOpen ? 'translate-x-0' : 'translate-x-full lg:translate-x-0'"
            ...
            <div class="flex items-center justify-center h-20 bg-white border-b border-gray-200 shadow-sm">
                ...
                <div class="flex items-center justify-center h-20 bg-white border-b border-gray-200 shadow-sm">
                    ...
                    <div>
                        ...
                        <p class="text-[10px] text-red-600 uppercase tracking-widest font-semibold">{{ __('Dashboard') }}</p>
                    </div>
                </div>
            </div>
            ...
            <nav class="flex-1 px-4 py-6 space-y-1 overflow-y-auto">
                <p class="px-4 text-xs font-semibold text-gray-500 uppercase tracking-wider mb-3">{{ __('Main Menu') }}</p>
            ...
            <div class="p-4 bg-gray-100 border-t border-gray-200">
                <div class="flex items-center gap-3">
                    ...
                    <div class="flex flex-col">
                        <span class="text-sm font-medium text-gray-800 truncate w-32">{{ auth()->user()->name ?? __('System Manager') }}</span>
                        <span class="text-xs text-red-600 font-medium">{{ __('Online') }}</span>
                    </div>
                </div>
            </div>
        </aside>
                ...
                <div class="flex items-center justify-between px-6 h-full">
                    ...
                    <div class="flex items-center gap-4">
                        ...
                        <div class="relative" x-data="{ open: false }">
                            <button @click="open = !open" class="flex items-center gap-2 text-sm text-gray-700 hover:text-red-600 transition-colors focus:outline-none">
                                <span class="hidden md:block">{{ app()->getLocale() == 'ar' ? 'العربية' : 'English' }}</span>
                                <svg class="w-4 h-4 text-gray-500" :class="{'rotate-180': open}" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/>
                                </svg>
                            </button>
                            <div x-show="open" @click.away="open = false" class="absolute left-0 mt-2 w-32 bg-white border border-gray-200 rounded-lg shadow-xl py-1 z-50 origin-top-left" x-cloak>
                                <a href="{{ route('switch-language', 'ar') }}" class="block w-full text-right px-4 py-2 text-sm text-gray-700 hover:bg-red-50 hover:text-red-700 transition-colors">العربية</a>
                                <a href="{{ route('switch-language', 'en') }}" class="block w-full text-right px-4 py-2 text-sm text-gray-700 hover:bg-red-50 hover:text-red-700 transition-colors">English</a>
                            </div>
                        </div>
