<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
    ...
    <nav class="mb-8 text-sm font-medium">
        ...
        <a href="{{ route('categories.index') }}" class="text-gray-500 hover:text-red-600 transition-colors">{{ __('Categories') }}</a>
        <span class="mx-2 text-gray-300">/</span>
        <span class="text-red-600 font-bold">{{ $category->name }}</span>
    </nav>
    ...
    <div class="bg-white rounded-3xl shadow-xl p-8 mb-12 border border-gray-100 relative overflow-hidden">
        ...
        <div class="relative z-10 flex flex-col md:flex-row items-center gap-8 text-center md:text-left">
            @if($category->image)
                <div class="w-32 h-32 md:w-40 md:h-40 flex-shrink-0 bg-white rounded-full p-2 shadow-lg border-4 border-red-50 overflow-hidden">
                    <img src="{{ asset('storage/' . $category->image) }}"
                         alt="{{ $category->name }}"
                         class="w-full h-full object-cover rounded-full hover:scale-110 transition-transform duration-500">
                </div>
            @else
                <div class="w-32 h-32 md:w-40 md:h-40 flex-shrink-0 bg-gradient-to-br from-red-500 to-red-600 rounded-full flex items-center justify-center shadow-lg border-4 border-red-50 text-white">
                    <span class="text-4xl font-black">{{ mb_substr($category->name, 0, 1) }}</span>
                </div>
            @endif
            <div class="flex-1">
                <h1 class="text-4xl md:text-5xl font-black text-gray-900 mb-4 tracking-tight">{{ $category->name }}</h1>
                @if($category->description)
                    <p class="text-lg text-gray-600 leading-relaxed max-w-3xl">{{ $category->description }}</p>
                @endif
            </div>
        </div>
    </div>
                    ...
                    <div class="p-6">
                        ...
                        <div class="flex items-center justify-between pt-4 border-t border-gray-100">
                            <div class="flex items-center gap-2">
                                ...
                                <span class="text-sm font-semibold text-gray-600">{{ $product->category->name }}</span>
                            </div>
